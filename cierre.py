...
from logica import guardar_movimiento, calcular_arqueo_fisico

def renderizar_cierre_caja(supabase_client, efectivo_caja_acumulado, movimientos_hoy):
    """Renderiza el Panel de Cierre de Caja y Arqueo General según la referencia visual"""
    ...
    with col1:
        ...
        if v_aran is not None:
            if round(v_aran, 2) == round(tot_aran_hoy, 2):
                ...
                st.error(f"❌ Diferencia: ${round(v_aran - tot_aran_hoy, 2):,.2f} (Sistema: ${tot_aran_hoy:,.2f})")
        ...
        if v_sell is not None:
            if round(v_sell, 2) == round(tot_sell_hoy, 2):
                ...
                st.error(f"❌ Diferencia: ${round(v_sell - tot_sell_hoy, 2):,.2f} (Sistema: ${tot_sell_hoy:,.2f})")
        ...
        if v_pate is not None:
            if round(v_pate, 2) == round(tot_pate_hoy, 2):
                ...
                st.error(f"❌ Diferencia: ${round(v_pate - tot_pate_hoy, 2):,.2f} (Sistema: ${tot_pate_hoy:,.2f})")
        
        st.markdown("---")
        st.markdown("#### 🏦 Depositar en Banco (Retiro)")
        monto_banco = st.number_input("Monto exacto a enviar al banco:", min_value=0.0, value=None, key="monto_banco")
        
        if st.button("Confirmar Depósito Bancario", use_container_width=True):
            if monto_banco and monto_banco > 0:
                datos_retiro = {
                    "detalle": "Depósito en Banco (Retiro de Efectivo)",
                ...
                if exito:
                    st.success("Retiro bancario asentado correctamente.")
                    st.rerun()
                else:
                    st.error(msj)
            else:
                st.warning("Ingrese un monto superior a 0.")
    ...
    with col3:
        st.markdown("#### 📊 3. Resultado de la Auditoría")
        st.metric(label="Efectivo esperado acumulado", value=f"${efectivo_caja_acumulado:,.2f}")
        st.metric(label="Efectivo real contado", value=f"${efectivo_real_contado:,.2f}")
        
        auditoria_dif = round(efectivo_real_contado - efectivo_caja_acumulado, 2)
        
        if auditoria_dif == 0:
            st.success("✅ AUDITORÍA COMPLETA: Caja perfectamente cuadrada.")
        elif auditoria_dif > 0:
            st.info(f" 🟩 SOBRANTE DE EFECTIVO:\n${auditoria_dif:,.2f}")
        else:
            st.error(f" 🟥 FALTANTE DE EFECTIVO:\n${abs(auditoria_dif):,.2f}")
